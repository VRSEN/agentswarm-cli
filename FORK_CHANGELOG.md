...
- **Downstream product profile inputs**
  ...
  - Behavior: the TUI sidebar shows the configured product version when present and falls back to the Agent Swarm installation version otherwise.
  - Behavior: when a product version is set, auto-update checks compare the registry version against the product version instead of the binary version, and only a strictly newer registry version can notify or auto-update. The Agent Swarm default still compares the binary version.
  ...
  - Implementation: `AgencyProduct` in `packages/opencode/src/agency-swarm/product.ts`, the sidebar footer in `packages/opencode/src/cli/cmd/tui/routes/session/sidebar.tsx`, launcher detection in `packages/opencode/src/agency-swarm/npx.ts`, add-on env storage in `packages/opencode/src/cli/cmd/tui/util/env-file.ts`, the FastAPI server launcher module argument, installation distribution metadata, update checks in `packages/opencode/src/cli/upgrade.ts`, native TUI provider dialogs, `packages/opencode/script/build.ts`, and `packages/opencode/script/generate.ts`.
...
- **Build can propose switching to Plan for unclear swarm work**
  ...
  - Behavior: Build never copies an existing parent-directory `.env`'s values into a new standalone agency, even with user confirmation. When a parent-directory `.env` exists, it inspects only key names and presence of values, then creates a template `.env` containing those key names plus the required key names from `.env.example`, all with empty values, and tells the user which keys to fill. When no parent-directory `.env` exists, the template uses only the required key names from `.env.example`. Secret values are never read into the conversation, exposed, or written into the new agency's files.
...
- **Privacy-safe product telemetry for TUI feature usage**
  ...
  - Behavior: npm release builds can send PostHog events for provider demand, provider auth started/failed outcomes, task success/failure for normal TUI prompts, supported slash command usage, docs clicks, project initialization, agent creation, and integration requested when the release build includes the official embedded PostHog capture key. Runtime user-supplied telemetry keys do not redirect production telemetry. Users can disable this with `ENABLE_TELEMETRY=0`, `OPEN_SWARM_TELEMETRY=0`, `AGENTSWARM_TELEMETRY=0`, or `--no-telemetry`.
  - Behavior: product launchers can attach explicit allowlisted marketplace metadata to telemetry events: `swarm_id`, `parent_swarm_id`, and `swarm_origin` (`original`, `fork`, or `unknown`). This lets marketplace run metrics distinguish original swarms from forks without reading raw git remotes or project paths.
  - Privacy: telemetry uses strict event and property allowlists; unknown events or properties are dropped or bucketed, provider and integration values are allowlisted, and captured properties exclude raw prompts, slash command arguments, command bodies, credentials, paths, project IDs, session IDs, message IDs, raw model IDs, source content, environment values, raw error text, agent names, agent descriptions, generated prompts, tool inputs, and tool outputs.
  - Documentation: `README.md` lists the supported events and properties and the derived dashboard metrics for first run, retention, sessions per user, first task, first successful task, time to first success, completed tasks, docs clicks, provider demand, project setup, and agent creation.
  - Implementation: `Telemetry` in `packages/opencode/src/telemetry/telemetry.ts`, command value allowlists in `packages/opencode/src/telemetry/command-values.ts`, TUI auth/prompt/add-on hooks, project initialization and agent creation hooks, README event documentation, and release-time `POSTHOG_API_KEY`/`POSTHOG_HOST` injection in `.github/workflows/publish-npm-on-release.yml`.

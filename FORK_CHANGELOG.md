...
- **Privacy-safe product telemetry for TUI feature usage**
  ...
  - Behavior: npm release builds can send PostHog events for provider demand, provider auth started/failed outcomes, task success/failure for normal TUI prompts, project initialized, and integration requested when the release build includes the PostHog capture key. Users can disable this with `OPEN_SWARM_TELEMETRY=0`, `AGENTSWARM_TELEMETRY=0`, or `--no-telemetry`.
  - Privacy: telemetry uses strict event and property allowlists; unknown events or properties are dropped or bucketed, provider and integration values are allowlisted, and captured properties exclude raw prompts, credentials, paths, project IDs, session IDs, message IDs, raw model IDs, source content, environment values, raw error text, tool inputs, and tool outputs.

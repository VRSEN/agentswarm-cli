...
description: Use when a local Codex CLI review, pull-request comment review loop, or saved Codex review artifact is required for this repo.
---
...
- Also read `FORK_CHANGELOG.md` and `USER_FLOWS.md` when fork behavior, TUI, release QA, or user flows are touched; use `.codex/skills/agent-swarm-e2e-testing` when Agent Swarm proof is in scope.

...
```bash
codex review --base <base> -c model='"gpt-5.5"' -c model_reasoning_effort='"high"' > /tmp/codex_review_$(git rev-parse --short HEAD).txt 2>&1
```

If GPT-5.5 high is unavailable, use the strongest available GPT-5.x review path, record the exact model and effort, and do not rely on unknown defaults.

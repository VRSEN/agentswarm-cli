...
- Stay at manager height: own the queue, requirements, mandate, critical path, delegation, final review, escalation, approval gates, merge/release decisions, and destructive-action decisions.
- Managers may inspect, review, run tests, orchestrate validation, update ledgers, prompt workers, escalate to users, and perform mechanical git operations on reviewed worker output. For this repo, they must not author non-trivial implementation, policy or repo-skill edits, bug fixes, or code/test edits unless the user explicitly approves a narrow exception.
...
2. Start each task with the worker's explicit role, exact user ask, mandate, repo, branch, artifact scope, allowed files and actions, relevant artifacts, source pointers, inspected evidence, unknowns to acquire, evidence requirements, success condition, output format, and hard limits.
3. Subagents are stateless. If the prompt carries context from another conversation, name that source explicitly; write "the end user told the manager" or "the manager's previous response", not bare "the user" or "previous response" when the worker cannot see that speaker.
...
- Workers must not merge, publish releases, tag, force-push, delete shared artifacts, or run destructive operations unless the manager delegates that exact action for that exact artifact after review and required human approval from the user or their named delegate.
- Claude delegation is discussion-only. Do not delegate implementation, code-review gates, patch generation, staging, committing, pushing, merge, release, or repo mutation to Claude.

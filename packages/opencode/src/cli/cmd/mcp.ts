...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro(`Add servers with: ${AgencyProduct.cmd} mcp add`)
      return
    }
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check branded config first and keep legacy files as fallback.
  const candidates = [
    ...ConfigPaths.fileInDirectory(baseDir, AgencyBrand.config),
    ...ConfigPaths.fileInDirectory(baseDir, AgencyBrand.legacyConfig),
  ]

  if (!global) {
    candidates.push(
      ...ConfigPaths.fileInDirectory(path.join(baseDir, AgencyBrand.workspace), AgencyBrand.config),
      ...ConfigPaths.fileInDirectory(path.join(baseDir, AgencyBrand.legacyWorkspace), AgencyBrand.legacyConfig),
    )
  }
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: `e.g., ${AgencyProduct.cmd} x @modelcontextprotocol/server-filesystem`,
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })

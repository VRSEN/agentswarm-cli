...
import { NotFoundError, Service as StorageService } from "@/storage/storage"
...
export const layer: Layer.Layer<
  Service,
  never,
  BackgroundJob.Service | Bus.Service | StorageService | SyncEvent.Service | RuntimeFlags.Service
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      if (runProject) {
        ...
        const remoteConfig = AgencySwarmRunSession.remoteConfigFromEnv({ directory: ctx.directory })
        if (remoteConfig) {
          yield* Effect.promise(() =>
            AgencySwarmRunSession.markRemote({ sessionID: result.id, directory: ctx.directory, config: remoteConfig }),
          )
        }
      }
      return result

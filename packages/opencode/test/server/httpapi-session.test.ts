...
function expectV2SessionActionNotImplemented(path: string) {
  const responses = OpenApi.fromApi(PublicApi).paths[path]?.post?.responses
  expect(responses?.["501"], `${path} must declare its unsupported response`).toBeTruthy()
}

function waitForUserMessageCount(sessionID: SessionIDType, count: number) {
  return Effect.gen(function* () {
...
describe("session HttpApi", () => {
  ...
  it.instance(
    "serves usable v2 session pagination cursors",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-opencode-directory": test.directory }
        yield* createSession({ title: "v2 cursor first" })
        yield* createSession({ title: "v2 cursor second" })
        yield* createSession({ title: "v2 cursor third" })

        const first = yield* requestJson<{
          items: Array<{ id: string }>
          cursor: { next?: string; previous?: string }
        }>("/api/session?limit=1", { headers })
        expect(first.cursor.next).toBeTruthy()
        const firstNext = first.cursor.next
        if (!firstNext) throw new Error("missing first next cursor")

        const second = yield* requestJson<{
          items: Array<{ id: string }>
          cursor: { next?: string; previous?: string }
        }>(`/api/session?limit=1&cursor=${encodeURIComponent(firstNext)}`, { headers })
        expect(second.cursor.next).toBeTruthy()
        const secondNext = second.cursor.next
        if (!secondNext) throw new Error("missing second next cursor")

        const third = yield* requestJson<{
          items: Array<{ id: string }>
          cursor: { next?: string; previous?: string }
        }>(`/api/session?limit=1&cursor=${encodeURIComponent(secondNext)}`, { headers })

        expect(first.items[0]?.id).toBeTruthy()
        expect(second.items[0]?.id).toBeTruthy()
        expect(third.items[0]?.id).toBeTruthy()
  ...
  it.instance(
    "scopes v2 session lists to the active project",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-opencode-directory": test.directory }
        const current = yield* createSession({ title: "current project" })
        const otherProject = ProjectID.make("project-v2-list-other")
        const otherSession = SessionID.descending()
        const now = Date.now()
        ...
        Database.use((db) => {
          db.insert(ProjectTable)
            .values({
              id: otherProject,
              worktree: "/tmp/opencode-other-project",
              sandboxes: [],
              time_created: now,
              time_updated: now,
            })
          ...
          db.insert(SessionTable)
            .values({
              ...
              slug: "other-project",
              directory: "/tmp/opencode-other-project",
              path: "",
              title: "other project",
              version: "test",
              cost: 0,
        ...
        const response = yield* requestJson<{ items: Array<{ id: string }> }>("/api/session", { headers })
  ...
  it.instance(
    "returns unsupported for v2 session actions",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-opencode-directory": test.directory, "content-type": "application/json" }
        const session = yield* createSession({ title: "v2 prompt" })

        const response = yield* request(`/api/session/${session.id}/prompt`, {
          method: "POST",
          headers,
          body: JSON.stringify({ prompt: { text: "hello v2" } }),
        })

        expect(response.status).toBe(501)
        expect(
          (yield* requestJson<{ items: SessionMessage.Message[] }>(`/api/session/${session.id}/message`, {
            headers,
          })).items,
        ).toEqual([])
        ...
        expect(wait.status).toBe(501)

        expectV2SessionActionNotImplemented("/api/session/{sessionID}/prompt")
        expectV2SessionActionNotImplemented("/api/session/{sessionID}/compact")
        expectV2SessionActionNotImplemented("/api/session/{sessionID}/wait")
      }),
    { git: true, config: { formatter: false, lsp: false } },
  )
  ...
  it.instance(
    "deletes queued Run-mode message while active turn is busy",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
          ...
          agencies: [
            {
              id: "local-agency",
              name: "Local Agency",
              agents: [{ id: "entry-agent", name: "entry-agent", isEntryPoint: true }],
              metadata: {},
            },
          ],
          rawOpenAPI: {},
        ...
        const users = yield* waitForUserMessageCount(session.id, 2)
        ...
        finishFirst.resolve()
        expect((yield* Effect.promise(() => Promise.resolve(first))).status).toBe(200)
        expect((yield* Effect.promise(() => Promise.resolve(second))).status).toBe(200)

...
describe("agency-swarm npx onboarding", () => {
  ...
  test("prepareProjectLaunch refreshes agency-swarm after manifest installs before the canary", async () => {
    ...
    spyOn(Bun, "spawn").mockImplementation((options: any) => {
      ...
      if (cmd.includes("pip") || isCanaryCommand(cmd)) {
        return {
          exited: Promise.resolve(isCanaryCommand(cmd) ? 1 : 0),
          stdout: "",
          stderr: isCanaryCommand(cmd) ? canaryStderr : "",
        } as never
      }
      throw new Error(`Unexpected command: ${cmd.join(" ")}`)
    })
    ...
    const canaryIndex = commands.findIndex((cmd) => isCanaryCommand(cmd))

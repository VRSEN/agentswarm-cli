...
test("plugin model hooks run for configured providers", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".opencode")
      const root = path.join(configDir, "plugin")
      ...
      await markPluginDependenciesReady(Global.Path.config)
      await Bun.write(
        path.join(root, "dynamic-provider.ts"),
        [
          "export default {",
          '  id: "demo.dynamic-provider",',
          "  server: async () => ({",
          "    async config(cfg) {",
          "      cfg.provider ??= {}",
          "      cfg.provider.demo = {",
  ...
  const provider = providers[ProviderID.make("demo")]
  expect(provider).toBeDefined()
  expect(provider.models[ModelID.make("dynamic")]?.name).toBe("Dynamic Model")
})

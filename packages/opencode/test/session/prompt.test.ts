      ...
      agencies: [
        {
          ...
          name: "Local Agency",
          agents: [
            { id: "MathAgent", name: "MathAgent", isEntryPoint: true },
            { id: "support_agent", name: "support_agent", isEntryPoint: false },
          ],
          metadata: {},
        },
      ],
...
unix(
  "shell projects completed output into v2 shell message",
  () =>
    Effect.gen(function* () {
      const { prompt, chat } = yield* boot()
      yield* prompt.shell({
        sessionID: chat.id,
        agent: "build",
        command: "printf v2-shell-output",
      })

      const messages = yield* SessionV2.Service.use((session) =>
        session.messages({ sessionID: chat.id, order: "asc" }),
      ...
      expect(shell.command).toBe("printf v2-shell-output")
      expect(shell.output).toBe("v2-shell-output")
...
it.instance(
  "unknown model stays a recoverable failure",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(ProviderSvc.ModelNotFoundError.isInstance(err)).toBe(true)
      }
    }),
  { git: true, config: cfg },
  30_000,

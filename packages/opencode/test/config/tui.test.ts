...
it.instance("loads .agentswarm/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeWithDirs(
        path.join(test.directory, ".agentswarm", "tui.json"),
        JSON.stringify({ diff_style: "stacked" }, null, 2),
      )
...
it.instance("branded .agentswarm/tui.json overrides same-level legacy .opencode/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeWithDirs(path.join(test.directory, ".agentswarm", "tui.json"), JSON.stringify({ theme: "brand" }))
      yield* fs.writeWithDirs(path.join(test.directory, ".opencode", "tui.json"), JSON.stringify({ theme: "legacy" }))
...
it.instance("explicit branded workspace tui config still overrides same-level legacy workspace tui config", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      ...
      const config = yield* withEnv("OPENCODE_CONFIG_DIR", path.join(test.directory, ".agentswarm"), getTuiConfig(test.directory))
      expect(config.theme).toBe("brand")
    }),
  ),
)

...
- Treat the hardcoded `Current fork merge commit` in `FORK_CHANGELOG.md` as the fork-delta anchor for edit checks; minimize each edit's diff against that merge commit, and keep the fork delta limited to Agency Swarm integration, required fork packaging or release work, and approved branding.
- Before any non-trivial edit to a file that also exists in upstream, read the upstream version first and prove that the change is fork-owned or directly caused by fork changes. Do not change upstream-owned behavior merely because an upstream bug exists. Ask: can you shape the change so the next upstream merge is easier, and do any changed lines look accidental or unexplained? If yes, treat those lines as a bug candidate, check `git blame` or `FORK_CHANGELOG.md`, and escalate to the user if you still cannot explain them.
- Triage every bug by ownership before fixing it. If the bug lives in upstream-owned OpenCode behavior, do not patch the fork around it; escalate to the user and recommend syncing or merging the latest upstream `dev` commits that may already contain the fix.
- If the bug is caused by fork customization documented in `USER_FLOWS.md` or `FORK_CHANGELOG.md`, fix it in a focused minimal pull request while aligning the fork code back to upstream machinery, reusing upstream implementation where possible, and keeping the smallest possible upstream delta.
...
- No tag, GitHub Release, or npm publish may happen without a green independent pre-release review of the exact release commit against the fork default branch; if any review finding remains, stop and surface it to the user.

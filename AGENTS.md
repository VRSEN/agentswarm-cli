...
- Build-impact changes must go through a pull request before they merge to `vrsen/dev` or ship in a release. Build-impact includes runtime code, packaging, release scripts, generated binaries, dependency manifests or lockfiles, CI/build workflow changes, and tests or harnesses that gate shipped behavior.
- Do not commit build-impact changes directly to `vrsen/dev` or another default branch unless the user explicitly approves an emergency exception for that exact change.
